"""
Unit tests for XGBoost Signal Source.

Tests the XGBoost-based trading signal source including:
...
@pytest.fixture
def xgboost_config(temp_model_dir):
    """Create XGBoost config for testing."""
...
class TestXGBoostSignalSourceInitialization:
    """Tests for XGBoostSignalSource initialization."""

    def test_initialization_default_config(self):
        """Test initialization with default configuration."""
        source = XGBoostSignalSource(symbol="V10")

        assert source.symbol == "V10"
        assert isinstance(source.config, XGBoostConfig)
        assert source.model is None
        assert not source._is_trained
        assert not source._is_initialized
        assert source.model_version == ""

    def test_initialization_custom_config(self, xgboost_config):
        """Test initialization with custom configuration."""
        source = XGBoostSignalSource(symbol="V25", config=xgboost_config)

        assert source.symbol == "V25"
        assert source.config == xgboost_config
        assert source.config.N_ESTIMATORS == 10

    def test_get_model_info_before_training(self, xgboost_config):
        """Test getting model info before training."""
        source = XGBoostSignalSource(symbol="V50", config=xgboost_config)
...
class TestXGBoostSignalSourceTraining:
    """Tests for XGBoost model training."""

    @pytest.mark.asyncio
    async def test_train_with_valid_data(self, sample_ohlcv_data, xgboost_config):
        """Test training with valid OHLCV data."""
        source = XGBoostSignalSource(symbol="V10", config=xgboost_config)
    ...
    @pytest.mark.asyncio
    async def test_train_with_empty_data(self, xgboost_config):
    ...
    @pytest.mark.asyncio
    async def test_train_with_missing_columns(self, xgboost_config):
    ...
    @pytest.mark.asyncio
    async def test_feature_importance_generated(self, sample_ohlcv_data, xgboost_config):
    ...
    @pytest.mark.asyncio
    async def test_get_feature_importance(self, sample_ohlcv_data, xgboost_config):
...
class TestXGBoostSignalSourcePrediction:
    """Tests for XGBoost prediction generation."""

    @pytest.mark.asyncio
    async def test_predict_after_training(self, sample_ohlcv_data, xgboost_config):
        ...
        assert signal.source == f"xgboost_V10"
    ...
    @pytest.mark.asyncio
    async def test_predict_without_training_fails(self, xgboost_config):
    ...
    async def test_prediction_direction_thresholds(self, sample_ohlcv_data):
        ...
        config = XGBoostConfig(BUY_THRESHOLD=0.9, SELL_THRESHOLD=0.9)
    ...
    @pytest.mark.asyncio
    async def test_prediction_metadata(self, sample_ohlcv_data, xgboost_config):
...
class TestXGBoostSignalSourcePersistence:
    """Tests for model save/load functionality."""

    @pytest.mark.asyncio
    async def test_save_model_after_training(self, sample_ohlcv_data, xgboost_config, temp_model_dir):
        ...
        model_file = Path(temp_model_dir) / f"{xgboost_config.MODEL_VERSION_PREFIX}_V10.json"
    ...
    @pytest.mark.asyncio
    async def test_load_existing_model(self, sample_ohlcv_data, xgboost_config, temp_model_dir):
        """Test loading an existing trained model."""
        # Train and save
        source1 = XGBoostSignalSource(symbol="V10", config=xgboost_config)
        await source1.train(sample_ohlcv_data)

        # Create new instance and load
        source2 = XGBoostSignalSource(symbol="V10", config=xgboost_config)
    ...
    @pytest.mark.asyncio
    async def test_initialize_loads_existing_model(self, sample_ohlcv_data, xgboost_config):
    ...
    @pytest.mark.asyncio
    async def test_initialize_with_force_retrain(self, sample_ohlcv_data, xgboost_config):
...
class TestXGBoostSignalSourceFeatures:
    """Tests for feature engineering integration."""

    @pytest.mark.asyncio
    async def test_uses_configured_features(self, sample_ohlcv_data, xgboost_config):
        """Test that model uses configured features."""
        source = XGBoostSignalSource(symbol="V10", config=xgboost_config)
    ...
    @pytest.mark.asyncio
    async def test_required_features_present(self, sample_ohlcv_data, xgboost_config):
        """Test that all required features from PRD are present."""
        source = XGBoostSignalSource(symbol="V10", config=xgboost_config)

        await source.train(sample_ohlcv_data)

        # PRD requires: RSI, MACD, ATR, moving averages, price momentum, volume
...
class TestConvenienceFunctions:
    ...
    def test_create_xgboost_signal_source(self):
        """Test factory function."""
        source = create_xgboost_signal_source(symbol="V10")

        assert source.symbol == "V10"
        assert isinstance(source, XGBoostSignalSource)

    def test_create_xgboost_signal_source_with_config(self, xgboost_config):
        """Test factory function with config."""
        source = create_xgboost_signal_source(symbol="V10", config=xgboost_config)

        assert source.symbol == "V10"
        assert source.config == xgboost_config

    @pytest.mark.asyncio
    async def test_create_and_train_source(self, sample_ohlcv_data):
        """Test async factory function."""
        source = await create_and_train_source(
            symbol="V10",
...
class TestWinRateAcceptanceCriteria:
    ...
    @pytest.mark.asyncio
    async def test_win_rate_on_test_set(self, sample_ohlcv_data, xgboost_config):
    ...
    @pytest.mark.asyncio
    async def test_model_metrics_completeness(self, sample_ohlcv_data, xgboost_config):
...
class TestXGBoostSignalSourceIntegration:
    """Integration tests for XGBoost signal source."""

    @pytest.mark.asyncio
    async def test_full_workflow(self, sample_ohlcv_data, xgboost_config):
    ...
    @pytest.mark.asyncio
    async def test_multiple_symbols(self, sample_ohlcv_data, xgboost_config):
        """Test training models for multiple symbols."""
        symbols = ["V10", "V25", "V50"]
        sources = []

        for symbol in symbols:
            source = XGBoostSignalSource(symbol=symbol, config=xgboost_config)
            await source.train(sample_ohlcv_data)
        ...
        for i, source in enumerate(sources):
            ...
            model_file = Path(xgboost_config.MODEL_DIR) / f"{xgboost_config.MODEL_VERSION_PREFIX}_{symbols[i]}.json"
            assert model_file.exists()
